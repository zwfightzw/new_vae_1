...
class FullQDisentangledVAE(nn.Module):
    ...
    def encode_z(self, x):
        ...
        z_fwd_list = torch.zeros(batch_size, self.hidden_dim).to(self.device)
        ...
        for t in range(1, seq_size):
            ...
            for fwd_t in range(self.block_size):
                ...
                '''
                z_fwd_list = self.z_to_c_fwd_list[fwd_t](zt_1_tmp, z_fwd_list, w=wt[:, fwd_t].view(-1, 1))

            curr_layer[t] = z_fwd_list
            # p(xt|zt)
            ...
            wt = self.z_w_function(z_fwd_list)
            wt = self.cumsoftmax(wt, self.temperature)

            store_wt.append(wt[0].detach().cpu().numpy())

            z_prior_fwd = self.z_prior_out_list(z_fwd_list)
            z_fwd_latent_mean = z_prior_fwd[:, :self.z_dim]
            z_fwd_latent_lar = z_prior_fwd[:, self.z_dim:]
    ...
    def forward(self, x, temp):
        ...
        z_post_norm, z_prior_norm, z, store_wt, raw_outputs, outputs, lstm_out = self.encode_z(conv_x)
        recon_x = self.dec_obs(z.view(num_samples * seq_len, -1)).view(num_samples, seq_len, *x.size()[2:])
        return z_post_norm, z_prior_norm, z, recon_x, store_wt, raw_outputs, outputs, lstm_out


def loss_fn(dataset, original_seq, recon_seq, z_post_norm, z_prior_norm,  raw_outputs, outputs, alpha, beta, kl_weight, lstm_out, eta):
    if dataset == 'lpc':
...
class Trainer(object):
    ...
    def sample_frames(self, epoch, sample):
        with torch.no_grad():
            ...
            '''
            store_wt = []
            z_fwd_list = torch.zeros(len, self.model.hidden_dim).to(self.device)
            ...
            for t in range(1, self.model.frames):
                for fwd_t in range(self.model.block_size):
                    ...
                    '''
                    z_fwd_list = self.model.z_to_c_fwd_list[fwd_t](zt_1_tmp, z_fwd_list, w=wt[:, fwd_t].view(-1, 1))

                # update weight, w0<...<wd<=1, d means block_size

                z_prior_fwd = self.model.z_prior_out_list(z_fwd_list)
                ...
                wt = self.model.z_w_function(z_fwd_list)
...
if __name__ == '__main__':
    ...
    # method
    parser.add_argument('--method', type=str, default='Hybrid_3')
    # dataset
    ...
    trainer = Trainer(vae, device, train_loader, test_loader, epochs=FLAGS.max_epochs, batch_size=FLAGS.batch_size,
                      ...
                      alpha=FLAGS.alpha, beta=FLAGS.beta, kl_weight=FLAGS.kl_weight, eta=FLAGS.eta)
