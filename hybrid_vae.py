...
class FullQDisentangledVAE(nn.Module):
    def __init__(self, temperature, frames, z_dim, conv_dim, hidden_dim, block_size, channel, dataset, device):
        ...
        self.z_w1_function = nn.Linear(self.hidden_dim, self.block_size)
    ...
    def encode_z(self, x):
        ...
        store_wt1.append(wt1[0].detach().cpu().numpy())
        ...
        for t in range(0, seq_size):
            ...
            wt1 = self.cumsoftmax(wt1, self.temperature)
    ...
    def sample_frames(self, epoch, sample):
        with torch.no_grad():
            ...
            for t in range(0, self.model.frames):
                ...
                wt1 = self.model.cumsoftmax(wt1, self.model.temperature)
