...
class FullQDisentangledVAE(nn.Module):
    def __init__(self, temperature, frames, z_dim, conv_dim, hidden_dim, block_size, channel, dataset, device):
        ...
        self.z_to_c_fwd_list = [GRUCell(input_size=self.z_dim, hidden_size=self.z_dim//self.block_size).to(self.device)
            for i in range(self.block_size)]

        self.z_w_function = nn.Linear(self.z_dim, self.block_size) #nn.Sequential(nn.Linear(self.hidden_dim, self.hidden_dim//2),nn.ReLU(), nn.Linear(self.hidden_dim//2, self.block_size))
        self.z_w1_function = nn.Linear(self.z_dim, self.block_size)
...
class Trainer(object):
    ...
    def sample_frames(self, epoch, sample):
        with torch.no_grad():
            ...
            wt1 = torch.ones(len, self.model.block_size).to(self.device)
            '''
            
            store_wt.append(wt[0].detach().cpu().numpy())
            '''
            store_wt = []
            z_fwd_list = [torch.zeros(len, self.model.z_dim//self.model.block_size).to(self.device) for i in range(self.model.block_size)]
            ...
            for t in range(0, self.model.frames):
                for fwd_t in range(self.model.block_size):
                    ...
                    if fwd_t == 0:
                        ...
                        zt_1_tmp = concat(torch.zeros(len, (self.model.block_size-2)*each_block_size).to(self.device),
                ...
                wt = cumsoftmax(wt)
                wt1 = self.model.z_w1_function(z_fwd_all)
                wt1 = cumsoftmax(wt1)
                store_wt.append(wt[0].detach().cpu().numpy())
                # decode observation
                zt_1 = zt
